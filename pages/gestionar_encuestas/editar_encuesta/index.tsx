...
const SurveyForm: React.FC = () => {
    ...
    useEffect(() => {
        const fetchSurveyData = async () => {
            try {
                ...
                const response = await axios.get(`${process.env.BASE_URL}/survey/get_survey_by_session_id/${sessionIdAsNumber}`);

                if (response.data) {
                    form.setFieldsValue({
                        ...response.data,
                        birth_date: response.data.birth_date ? moment(response.data.birth_date) : null,
                    });
                }
            } catch (error) {
                if (axios.isAxiosError(error)) {
                    console.log('No se encontró una encuesta para esta sesión');
                } else {
    ...
    const handleSubmit = async (values: any) => {
        ...
        try {
            const existingSurvey = await axios.get(`${process.env.BASE_URL}/survey/get_survey_by_session_id/${sessionIdAsNumber}`);
            const surveyId = existingSurvey.data.id;
        ...
        <div style={{ padding: '20px', maxWidth: isTablet ? '720px' : '100%', margin: 'auto' }}>
            ...
            {loading ? (
                ...
                <Form form={form} layout="vertical" onFinish={handleSubmit}>
                    ...
                    <Row gutter={isTablet ? 8 : 16}>
                        <Col span={24}>
                            <Form.Item label="¿Le ha ayudado alguien a rellenar el cuestionario?" name="help_with_questionnaire" rules={[{ required: true }]}>
                                <TextArea rows={4} />
                            </Form.Item>
                        </Col>
                        <Col span={24}>
                            <Form.Item label="Tiempo para completar (minutos)" name="time_taken_to_complete" rules={[{ required: true, message: 'Ingrese el tiempo' }]}>
                                <TextArea rows={4} />
                            </Form.Item>
                        </Col>
                        <Col span={24}>
